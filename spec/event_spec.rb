...
RSpec.describe Event do
  ...
  describe 'initialize' do
    it 'raises ValidationError when name is not specified' do
      attrs = {date_time: DateTime.now, invites_limit: 10, customer: @customer}
    ...
    it 'raises ValidationError when invites_limit is not specified' do
      attrs = {name: 'Cool Party', date_time: DateTime.now, customer: @customer}
      expect{Event.new(attrs)}.to raise_error ValidationError
    end
    ...
    it 'creates an instance if all parameters are specified correctly' do
      attrs = {name: 'Cool Party', date_time: DateTime.now, invites_limit: 10, customer: @customer}
  ...
  describe 'add_invite' do
    context 'when invites_sent is less than invites_limit' do
      before :each do
        ...
        @event = Event.new(name: 'Event', date_time: DateTime.now, invites_limit: 5, customer: @customer)
      end
    ...
    context 'when invites_sent is equal to invites_limit' do
      before :each do
        @event = Event.new(name: 'Event', date_time: DateTime.now, invites_limit: 1, customer: @customer)
    ...
    context 'when invite with same email has already been added' do
      before :each do
        @event = Event.new(name: 'Event', date_time: DateTime.now, invites_limit: 100, customer: @customer)
        invite1 = double(:invite1)
